...
(once for each inbound link).  This ``overcounting'' leverages
syntactic indications of which words are more
important to translate correctly (e.g., ``Basra'' in the
example).

A probabilistic parser provides confidences associated with parses
in an $n$-best list, which we can use to compute
expected counts for each decomposition in both the reference and
hypothesized translations. The expected counts lead to partial matches
which are used to compute precision and recall as weighted counts.
Using the expectations can help cope with both error
...
rough\endnote{Our segment count differs from
  \inlinecite{owczarzak07evaluatingmt}, who report 16,807 segments
  over the same corpus, so the results presented here are not directly
  comparable with that paper. As a result, the baseline (BLEU and TER)
...
scores,\endnote{Previous work 
  ...
  derived from the same source segment as a way of factoring out 
  sentence difficulty, but this approach introduces problems with 
  the independence assumptions in the Pearson's $r$ tests.}
\begin{equation}
  \label{eq:meansub}
