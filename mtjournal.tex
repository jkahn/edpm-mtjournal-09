...
compare EDPM to other metrics in correlation with document-level HTER
performance. 
...
$s_{\textrm{HTER}}(t_{i,z})$ at both the sentence and document level.
...
subcorpora. EDPM's strengths are particularly clear in the
unstructured domains (\texttt{wb} and \texttt{bc}).  

In table~\ref{tab:hterpersent}, we see a similar story.
\begin{table}
  \begin{tabular}{r|rrrr|rr|r}
    \hline
    $r$ vs. $\bar{\mu}$HTER & \multicolumn{1}{c}{\texttt{bc}}
  ...
  \caption{Per-sentence, length-weighted correlations of
    ...
    source language. Bold numbers are within 95\% significance of the
    best per column.}
...
TO DO: NOTE RE: SEGMENT-LEVEL CORRS
